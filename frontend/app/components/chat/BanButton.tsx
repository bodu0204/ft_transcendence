...
export function BanButton({chanel_id, user_id}:{chanel_id:number,user_id:number}){
	...
	function ban(){
		if (!b || !taken)
			return ;
		set_b(_=>false);
		fetch(`/backend/channel/ban/${chanel_id}`, init).then(_=>{
			set_b(_=>true);
		});
	}
	return (
		<button className={Styles.bt_styles} onClick={ban}>{b?'ban':'banning'}</button>
	);
}
